...
locales.forEach((lang) => {
    fields[lang] = {
        title: props.aboutus.translations[lang]
            ? props.aboutus.translations[lang].title
            : "",
        description: props.aboutus.translations[lang]
            ? props.aboutus.translations[lang].description
            : "",
    };
});
const form = useForm({
    ...fields,
    status: false,
    image: props.aboutus.image,
});

const onSubmit = () => {
    form.put(route("admin.aboutus.update", props.aboutus.id), {
        onSuccess: () => {
...
<template>
    ...
    <AdminLayout>
        <template #header>
            ...
            >
                <h1 class="h3 mb-0 text-gray-800">{{ __("Edit About Us") }}</h1>
                <Link
